 ...
 *
 * Build: gcc -o shure-mv6-sync shure-mv6-sync.c $(pkg-config --cflags --libs libpulse)
 */
#include <dirent.h>
#include <signal.h>
#include <stdio.h>
...
static int g_syncing =  0;  /* set while we are pushing a change to prevent loops */
...
static int find_mute_path(char *out, size_t len)
{
    ...
    while ((e = readdir(d))) {
        ...
        snprintf(path, sizeof(path),
                 "/sys/bus/hid/devices/%s/../../idVendor", e->d_name);
        if (!(f = fopen(path, "r"))) continue;
        int ok = fgets(buf, sizeof(buf), f) != NULL;
        fclose(f);
        if (!ok || strncasecmp(buf, VENDOR_ID, strlen(VENDOR_ID)) != 0) continue;

        snprintf(path, sizeof(path),
                 "/sys/bus/hid/devices/%s/../../idProduct", e->d_name);
        if (!(f = fopen(path, "r"))) continue;
        ok = fgets(buf, sizeof(buf), f) != NULL;
        fclose(f);
        if (!ok || strncasecmp(buf, PRODUCT_ID, strlen(PRODUCT_ID)) != 0) continue;

        snprintf(path, sizeof(path),
                 "/sys/bus/hid/devices/%s/mute", e->d_name);
...
static void source_info_cb(pa_context *c, const pa_source_info *info,
                           int eol, void *ud)
{
    if (eol || !info) return;

    int os = info->mute;
    if (os == g_last_os) return;
    g_last_os = os;
                         ...
                         uint32_t idx, void *ud)
{
    ...
    if (fac == PA_SUBSCRIPTION_EVENT_SOURCE &&
        ...
        pa_operation *op = pa_context_get_source_info_by_name(
                c, "@DEFAULT_SOURCE@", source_info_cb, NULL);
...
static void context_state_cb(pa_context *c, void *ud)
{
    ...
    case PA_CONTEXT_READY: {
        ...
        pa_operation *op = pa_context_subscribe(
                c, PA_SUBSCRIPTION_MASK_SOURCE, NULL, NULL);
        if (op) pa_operation_unref(op);
